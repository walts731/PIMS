...
// Check if user has correct role (main_user)
if ($_SESSION['role'] !== 'main_user') {
...
// Debug logging
error_log("MAIN_USER Notifications handler called: action=$action, user_id=" . $_SESSION['user_id']);
...
try {
    ...
    error_log("MAIN_USER Notifications handler error: " . $e->getMessage());
...
function getNotifications() {
    ...
    $sql = "SELECT n.*, 
                   CASE 
                       WHEN n.related_type = 'asset' THEN CONCAT('view_asset_item.php?asset_id=', n.related_id)
                       ELSE '#'
                   END as action_url
