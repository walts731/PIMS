...
function getOfficeAdminId($office_id) {
    ...
    $sql = "SELECT id FROM users WHERE role = 'office_admin' AND office_id = ? LIMIT 1";
    $stmt = $conn->prepare($sql);
...
function createLowStockNotification($office_id, $consumable_id, $consumable_name, $current_stock, $reorder_level) {
    ...
    $notification_id = createOfficeNotification($office_admin_id, $title, $message, 'warning', $consumable_id, 'consumable');
...
function createNewRequestNotification($office_id, $request_id, $request_type, $requester_name) {
    ...
    $notification_id = createOfficeNotification($office_admin_id, $title, $message, 'info', $request_id, 'request');
...
function createMaintenanceNotification($office_id, $asset_id, $asset_name) {
    ...
    $notification_id = createOfficeNotification($office_admin_id, $title, $message, 'warning', $asset_id, 'asset');
...
function createBorrowRequestNotification($office_id, $borrow_request_id, $requester_office, $asset_name) {
    ...
    $notification_id = createOfficeNotification($office_admin_id, $title, $message, 'info', $borrow_request_id, 'request');
...
function createConsumptionNotification($office_id, $consumable_id, $consumable_name, $quantity_consumed, $remaining_stock) {
    ...
    $notification_id = createOfficeNotification($office_admin_id, $title, $message, 'info', $consumable_id, 'consumable');
...
function createAssetStatusNotification($office_id, $asset_id, $asset_name, $old_status, $new_status) {
    ...
    $notification_id = createOfficeNotification($office_admin_id, $title, $message, $type, $asset_id, 'asset');
...
function checkLowStockNotifications($office_id) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $check_sql = "SELECT id FROM notifications 
                     WHERE user_id = (SELECT id FROM users WHERE role = 'office_admin' AND office_id = ? LIMIT 1)
...
function createOfficeSystemNotification($office_id, $title, $message, $type = 'system') {
    $office_admin_id = getOfficeAdminId($office_id);
    if (!$office_admin_id) return false;
    
    $notification_id = createOfficeNotification($office_admin_id, $title, $message, $type);
